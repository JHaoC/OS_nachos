...
Machine::Translate(int virtAddr, int *physAddr, int size, bool writing)
{
	...
	{
		// chech TLB and break is hit TLB
		for (entry = NULL, i = 0; i < TLBSize; i++)
		{
			if (tlb[i].threadID == kernel->currentThread->threadID && tlb[i].valid && (tlb[i].virtualPage == (int)vpn ))
			{
				entry = &tlb[i]; // FOUND!
				break;
			}
	...
	*physAddr = pageFrame * PageSize + offset;
	...
	// maintainning pageContainer for pagefault
	// Because it can run, the page must stay at the pageContain 
	// Remove it and append it back 
	if(kernel->PageContainer->IsInList(entry))
	{
		kernel->PageContainer->Remove(entry);
	}
	kernel->PageContainer->Append(entry);
