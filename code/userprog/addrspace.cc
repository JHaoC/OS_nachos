...
AddrSpace::AddrSpace()
{
    // page table will create at load function
    // std::cout << "Creating address space for " << kernel->currentThread->getName() << std::endl;
    ...
    // }
    // std::cout << "Created address space for " << kernel->currentThread->getName() << std::endl;

    // // zero out the entire address space
    // bzero(kernel->machine->mainMemory, MemorySize);
    // std::cout << "bzero address space for " << kernel->currentThread->getName() << std::endl;
}
...
AddrSpace::AddrSpace(const AddrSpace &a, int childthread)
{
    //std::cout << "Copy constructpr " << kernel->currentThread->getName() << std::endl;
    DEBUG(dbgAddr, "copy constructor"
                       << "\n");
    numPages = a.numPages;
    pageTable = new TranslationEntry[numPages];
    for (int i = 0; i < numPages; i++)
    {
        pageTable[i].virtualPage = a.pageTable[i].virtualPage;
        ...
        pageTable[i].readOnly = a.pageTable[i].readOnly;
        pageTable[i].threadID = childthread;
    }
...
bool AddrSpace::Load(char *fileName)
{
    ...
    // check if use header and make it if not.
    ...
    // Task 2 to deal with larger prog which latger than main memory
    // Rewrite loading load file into swap instead of loading at mainmemory directly
    if (noffH.code.size > 0)
    {
        ...
        for (int i = 0; i < numPages; i++)
        {
            ...
            // valid is false to involve pagefault
            pageTable[i].valid = FALSE;
            pageTable[i].use = FALSE;
            pageTable[i].dirty = FALSE;
            pageTable[i].readOnly = FALSE;
            pageTable[i].threadID = kernel->currentThread->threadID;
    ...
    //std::cout<<"Loaded the address space by "<<kernel->currentThread->getName()<<std::endl;
