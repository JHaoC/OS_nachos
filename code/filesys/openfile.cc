...
OpenFile::OpenFile(const OpenFile &f)
{
    hdr = new FileHeader;
    openfileId = f.openfileId;
    hdr->FetchFrom(openfileId);
    seekPosition = f.seekPosition;
}
...
{
    // // expend and update hdr
    // if(seekPosition + numBytes > hdr->FileLength())
    // {
    //     PersistentBitmap * tmp = new PersistentBitmap(NumSectors);
    //     tmp->FetchFrom(kernel->freeMapFile);
    //     hdr->Allocate(tmp,numBytes);
    //     hdr->WriteBack(indexOfSector);
    //     tmp->WriteBack(kernel->freeMapFile);
    //     delete tmp;
    // }
...
{
    // expend and update hdr
    while(position + numBytes > hdr->FileLength())
    {
        PersistentBitmap * tmp = new PersistentBitmap(NumSectors);
        tmp->FetchFrom(kernel->freeMapFile);
        hdr->Allocate(tmp,numBytes);
        hdr->WriteBack(openfileId);
        tmp->WriteBack(kernel->freeMapFile);
        delete tmp;
    }
